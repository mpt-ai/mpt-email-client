...
def _send_mail(args):
    r = requests.post(f"{args.get('host')}/mail/api/emailMessage/",
            headers={
...
def sendEmail(subject=None, message=None, emails=None, attachments=[], host=MPT_EMAIL_URI, api_key=MPT_EMAIL_APIKEY, ssl_verify=True):
    
    if host == None or api_key is None:
        raise Exception("please input `host` and `api_key` or set env `MPT_EMAIL_APIKEY` and `MPT_EMAIL_URI`")
